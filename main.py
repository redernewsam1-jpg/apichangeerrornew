            ...
            if not is_authorized:
                await m.reply_photo(
                    ...
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⌯ FʀᴏɴᴛMᴀɴ | ×͜× |", url="https://t.me/MrFrontMan001")],
                                                       [InlineKeyboardButton("ғᴇᴀᴛᴜʀᴇꜱ 🪔", callback_data="help"), InlineKeyboardButton("ᴅᴇᴛᴀɪʟꜱ 🦋", callback_data="help")]])
                )
                return
...
async def unauthorized_handler(client, message: Message):
    await message.reply(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💫 Get Premium Access", url="https://t.me/MrFrontMan001")]])
    )
...
async def send_logs(client: Client, m: Message):
    ...
    if m.chat.type == "channel":
        if not db.is_channel_authorized(m.chat.id, bot_username): return
    else:
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        ...
        await editable.edit("**⚠️ Timeout/Error! Using default thumbnail.**")
        await asyncio.sleep(1)
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                try:
                    url_api = f"https://covercel.vercel.app/extract_keys?url={url}@bots_updatee&user_id=7793257011"
                    mpd, keys = helper.get_mps_and_keys(url_api)
                    url = mpd
                    keys_string = " ".join([f"--key {key}" for key in keys])
                except Exception as e:
                    print(f"Testbook/Classplus API Error: {e}")
                ...
                headers = {'host': 'api.classplusapp.com', 'x-access-token': f'{raw_text4}', 'user-agent': 'Mobile-Android'}
                params = {"url": f"{url}"}
                try:
                    response = requests.get('https://api.classplusapp.com/cams/uploader/video/jw-signed-url', headers=headers, params=params)
                    url = response.json().get('url', url)
                except Exception as e:
                    print(f"Tencdn API Error: {e}")
                ...
                try:
                    ...
                    print(f"Videos Classplus API Error: {e}")
                ...
                try:
                    ...
                    print(f"Media CDN API Error: {e}")
            # --- END SAFE API PARSING FIXES ---
            ...
            if ".pdf*" in url: url = f"https://dragoapi.vercel.app/pdf/{url}"
            ...
            try:
                ...
                if "drive" in url:
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                if response.status_code == 200:
                                    with open(f'{name}.pdf', 'wb') as file: file.write(response.content)
                                    await asyncio.sleep(retry_delay)
                    ...
                    try:
                        ...
                        time.sleep(1)
                        await bot.send_document(chat_id=channel_id, document=f"{name}.html", caption=cc1)
                    ...
                    prog = await bot.send_message(channel_id, f"<i><b>Video APPX Encrypted Downloading</b></i>\n<blockquote><b>{str(count).zfill(3)}) {name1}</b></blockquote>", disable_web_page_preview=True)
                    try:
                        filename = await helper.download_and_decrypt_video(url, cmd, name, appxkey)  
                    ...
                    prog = await bot.send_message(channel_id, f"<i><b>📥 Fast Video Downloading</b></i>\n<blockquote><b>{str(count).zfill(3)}) {name1}</b></blockquote>")
                    filename = await helper.decrypt_and_merge_video(mpd, keys_string, path, name, raw_text2)
                    await prog.delete(True)
                    await helper.send_vid(bot, m, cc, filename, thumb, name, prog, channel_id, watermark=watermark)
                    count += 1
                    await asyncio.sleep(1)
    ...
    if raw_text7 == "/d":
        await bot.send_message(
            channel_id,
            f"<b>📬 ᴘʀᴏᴄᴇꜱꜱ ᴄᴏᴍᴘʟᴇᴛᴇᴅ</b>\n\n<blockquote><b>📚 ʙᴀᴛᴄʜ ɴᴀᴍᴇ :</b> {b_name}</blockquote>\n"
            f"╭────────────────\n├ 🖇️ ᴛᴏᴛᴀʟ ᴜʀʟꜱ : <code>{len(links)}</code>\n├ ✅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟ : <code>{success_count}</code>\n├ ❌ ꜰᴀɪʟᴇᴅ : <code>{failed_count}</code>\n╰────────────────\n\n"
            f"<i>ᴇxᴛʀᴀᴄᴛᴇᴅ ʙʏ ᴡɪᴢᴀʀᴅ ʙᴏᴛꜱ 🤖</i>"
        )
    else:
        await bot.send_message(channel_id, f"<b>-┈━═.•°✅ Completed ✅°•.═━┈-</b>\n<blockquote><b>🎯Batch Name : {b_name}</b></blockquote>\n<blockquote>🔗 Total URLs: {len(links)} \n┃   ┠🔴 Total Failed URLs: {failed_count}\n┃   ┠🟢 Total Successful URLs: {success_count}</blockquote>")
...
async def text_handler(bot: Client, m: Message):
    ...
    else: return await m.reply_text("<pre><code>Invalid link format.</code></pre>")
    ...
    quality = f"{raw_text2}p"
    await input2.delete(True)
    res = "854x480" if raw_text2 == "480" else "1280x720"
          
    raw_text4 = "working_token"
    thumb, count, channel_id = "/d", 0, m.chat.id
    try:
        url = link.replace("file/d/","uc?export=download&id=").replace("www.youtube-nocookie.com/embed", "youtu.be")
        name1 = links.replace("(", "[").replace(")", "]").strip()
        name = f'{name1[:60]}'
            
        # --- START SAFE PARSING FOR SINGLE LINKS ---
        if "classplusapp" in url:
            try:
                response = requests.get(f"https://covercel.vercel.app/extract_keys?url={url}@bots_updatee&user_id=7793257011", timeout=20)
                url = response.json().get('url', url)
            except Exception: pass
        elif "tencdn.classplusapp" in url:
            try:
                response = requests.get('https://api.classplusapp.com/cams/uploader/video/jw-signed-url', headers={'x-access-token': f'{raw_text4}'}, params={"url": f"{url}"})
                url = response.json().get('url', url)
            except Exception: pass
        elif 'videos.classplusapp' in url:
            try:
                url = requests.get(f'https://api.classplusapp.com/cams/uploader/video/jw-signed-url?url={url}', headers={'x-access-token': f'{raw_text4}'}).json().get('url', url)
            except Exception: pass
        ...
        # --- END SAFE PARSING FOR SINGLE LINKS ---

        if "youtu" in url: ytf = f"bv*[height<={raw_text2}][ext=mp4]+ba[ext=m4a]"
        else: ytf = f"b[height<={raw_text2}]/bv[height<={raw_text2}]+ba"
        cmd = f'yt-dlp -f "{ytf}" "{url}" -o "{name}.mp4"'
...
def notify_owner():
    requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", data={"chat_id": OWNER_ID, "text": "Bᴏᴛ Iꜱ Lɪᴠᴇ Nᴏᴡ 🤖"})
...
def reset_and_set_commands():
    ...
    commands = [
        ...
        {"command": "t2t", "description": "📝 ᴛᴇxᴛ → .ᴛxᴛ ɢᴇɴᴇʀᴀ"},
